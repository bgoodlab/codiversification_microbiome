...
for line in file:
	...
	if country=='Gabon' or continent=='Europe':
...
for s in [1,2]:
	...
	bacteria_countries[s] = []
	bacteria_continents[s] = []
	
	for i in range(0,len(samples)):
		bacteria_countries[s].append(sample_country_map[samples[i]])
		bacteria_continents[s].append(sample_continent_map[samples[i]])
		for j in range(i+1,len(samples)):
			...
			is_same_target = True #is_same_country and (sample_country_map[samples[i]]=='Gabon')
			
			same_country[s].append(is_same_country)
			same_continent[s].append(is_same_continent)
			same_target[s].append(is_same_target)
			...
			if (samples[i],samples[j]) not in host_host_samples:
				...
				host_same_target.append(is_same_target)
	...
	same_target[s] = numpy.array(same_target[s])
	...
	bacteria_countries[s] = numpy.array(bacteria_countries[s])
	bacteria_continents[s] = numpy.array(bacteria_continents[s])
	
for h in [1,2]:	
	host_host_ds[h] = numpy.array(host_host_ds[h])
host_same_target = numpy.array(host_same_target)
...
host_countries = numpy.array([sample_country_map[sample] for sample in samples])
host_continents = numpy.array([sample_continent_map[sample] for sample in samples])
...
bac_bac_continents = []
for i in range(0,len(samples)):
	bac_bac_countries.append(sample_country_map[samples[i]])
	bac_bac_continents.append(sample_continent_map[samples[i]])
	...
	for j in range(i+1,len(samples)):
		...
		bac_bac_same_target.append(is_same_target)

bac_bac_same_target = numpy.array(bac_bac_same_target)
...
bac_bac_countries = numpy.array(bac_bac_countries)
bac_bac_continents = numpy.array(bac_bac_continents)

for s in [0,1,2]:
	bac_bac_ds[s] = numpy.array(bac_bac_ds[s])
	

max_host_d = max([host_host_ds[h][host_same_target].max() for h in [1,2]]) 
max_bacteria_d = {s: (bacteria_ds[s][same_target[s]]).max() for s in [1,2]}
...
host_axis.set_ylabel('Host phylo distance (2/2)')
host_axis.set_xlabel('Host phylo distance (1/2)')
...
for s in reversed(range(1,5)):
	...
	if s==3: # the host host one
		...
		dys = bac_bac_ds[1]
		dxs = bac_bac_ds[2]
		target_idxs = bac_bac_same_target
		...
		countries = bac_bac_countries
		continents = bac_bac_continents
		...
		countries = bacteria_countries[s]
		continents = bacteria_continents[s]
	...
	axis.hist2d(dxs[target_idxs], dys[target_idxs], bins=(50, 50),norm=mcolors.PowerNorm(0.3),cmin=1,cmap="copper_r")
	
	line3, = axis.plot([-2],[-2],'o',label='Closest diff continent',markersize=3,color=other_color)
	line2, = axis.plot([-2],[-2],'o',label='Closest diff country',markersize=3,color=continent_color)
	...
	most_related = []
	other_most_related = []
	continent_most_related = []
	country_avg_related = []

	for i in range(0,dmatrixy.shape[0]):
		if countries[i]!='Gabon':
			continue
		...
		# Now do similar thing for bacteria from other countries
		good_idxs = (countries!=countries[i])*(continents==continents[i])*(dmatrixy[i,:]>0)
		if good_idxs.sum()>0:
			idx = numpy.argmin(dmatrixy[i,:]+1e06*(1-good_idxs))
			continent_most_related.append(dmatrixy[i,idx])
		
		# Now do same thing for other continent
		good_idxs = (continents!=continents[i])*(dmatrixy[i,:]>0)
		if good_idxs.sum()>0:
			idx = numpy.argmin(dmatrixy[i,:]+1e06*(1-good_idxs))
			other_most_related.append(dmatrixy[i,idx])
		
		good_idxs = (countries==countries[i])
		country_avg_related.extend(dmatrixy[i,good_idxs])
		
	hist_axis.hist(other_most_related, bins=(numpy.linspace(0,max_dy,50)-0.001),histtype='step',density=True,orientation='horizontal',color=other_color)
	hist_axis.hist(continent_most_related, bins=(numpy.linspace(0,max_dy,50)+0.001), histtype='step',density=True,orientation='horizontal',color=continent_color)
	hist_axis.hist(country_avg_related, bins=numpy.linspace(0,max_dy,50),histtype='step',density=True,orientation='horizontal',color=country_color)
	...
	for i in range(0,dmatrixy.shape[0]):
		...
		# Now do similar thing for hosts from other countries
		good_idxs = (countries!=countries[i])*(continents==continents[i])*(dmatrixx[i,:]>0)
		if good_idxs.sum()>0:
			idx = numpy.argmin(dmatrixx[i,:]+1e06*(1-good_idxs))
			continent_most_related.append(dmatrixx[i,idx])
		
		# Now do same thing for other continent
		good_idxs = (continents!=continents[i])*(dmatrixx[i,:]>0)
		if good_idxs.sum()>0:
			idx = numpy.argmin(dmatrixx[i,:]+1e06*(1-good_idxs))
			other_most_related.append(dmatrixx[i,idx])
		
		good_idxs = (countries==countries[i])
		country_avg_related.extend(dmatrixx[i,good_idxs])
	
	hhist_axis.hist(other_most_related, bins=(numpy.linspace(0,max_dx,50)-0.001),histtype='step',density=True,color=other_color)
	hhist_axis.hist(continent_most_related, bins=(numpy.linspace(0,max_dx,50)+0.001), histtype='step',density=True,color=continent_color)
	hhist_axis.hist(country_avg_related, bins=numpy.linspace(0,max_dx,50),histtype='step',density=True,color=country_color)
...
for s in [1,2]:
	...
	bacteria_axis[s].set_ylabel("%s phylo distance" % get_pretty_name(species[s]))
	bacteria_axis[s].set_xlabel('Host phylo distance')
...
bac_bac_axis.set_xlabel('%s phylo distance' % get_pretty_name(species[2]))
bac_bac_axis.set_ylabel('%s phylo distance' % get_pretty_name(species[1]))
...
print("Saving fig!")
fig.savefig('%s_%s_distance_correlation_closest.png' % (species[1],species[2]),bbox_inches='tight',dpi=300)
fig2.savefig('%s_%s_distance_cocorrelation_closest.png' % (species[1],species[2]),bbox_inches='tight',dpi=300)
